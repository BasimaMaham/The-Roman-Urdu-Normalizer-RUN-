...
with open("round3_eval_sheet.txt", "w", encoding="utf-8") as f:
    f.write("ROUND 3 DEVELOPMENT SET — Roman Urdu Normalizer\n")
    f.write("="*70 + "\n")
...
with open("round3_eval_data.json", "w", encoding="utf-8") as f:
    json.dump(data, f, ensure_ascii=False, indent=2)

changed = sum(1 for item in data if item['changed'])
print(f"Total: {len(data)} | Changed: {changed}")
print("Saved round3_eval_sheet.txt and round3_eval_data.json")